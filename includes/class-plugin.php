...
if ( ! class_exists( __NAMESPACE__ . '\Plugin' ) ) {
	class Plugin extends Singleton {
		...
		static public function get( $name = null ) {
			$path = WP_PLUGIN_DIR . '/cc-link-shortcode';
		...
		public function __construct() {
			...
			add_action( 'activated_plugin',   [ $this, 'switch_plugin_hook' ], 10, 2 );
			add_action( 'deactivated_plugin', [ $this, 'switch_plugin_hook' ], 10, 2 );
		...
		protected function is_hook( $method ) {
			foreach ( [ 'filter_', 'action_' ] as $hook ) {
		...
		protected function call_user_func_array( $function, $args = [] ) {
			if ( is_multisite() ) {
				foreach ( get_sites( [ 'public' => 1 ] ) as $blog ) {
